...
.DESCRIPTION
  ...
  Its intened use is to create a keytab file for a single service per server, i.e. SAS, HTTP etc
...
ForEach($keytab in $keytabs)
{
    ...
    $cmd = "ktpass -princ $service/$server@DA-ACH-DEV2.NP.AZ.DWPCLOUD.UK -mapuser $($mapuser) -mapOp set -crypto all ptype KRB5_NT_PRINCIPAL -pass DSPSuser31! -out c:\temp\$server-$service-krb5.keytab"
    invoke-expression $cmd
    $cmd = "ktpass -princ $service/$server.da-ach-dev2.np.az.dwpcloud.uk@DA-ACH-DEV2.NP.AZ.DWPCLOUD.UK -mapuser $($mapuser) -mapOp set -crypto all ptype KRB5_NT_PRINCIPAL -pass DSPSuser31!  -in c:\temp\$server-$service-krb5.keytab -out c:\temp\$server-$service-krb5.keytab"
    Invoke-Expression $cmd
    ...
    $kinit   = Invoke-SSHCommand -SSHSession $session -command "kinit -kt /tmp/$server-$service-krb5.keytab $service/$server.da-ach-dev2.np.az.dwpcloud.uk@$domain"
